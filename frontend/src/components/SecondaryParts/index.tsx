...
function SecondaryParts({ selectedMotherboardId, dispatch }: SecondaryPartsProps) {
    ...
    return (<>
        {!!motherboard.relatedCpus.length &&
            (<Selector<CPU> type="cpu" label="CPU" options={motherboard.relatedCpus} onChange={selectCPU} startingText='choose the cpu' />)}
        {!!motherboard.relatedRam.length &&
            (<Selector<RAM> type="ram" label="Ram" options={motherboard.relatedRam} onChange={selectRam} startingText='choose the ram' />)}
        {!!motherboard.relatedGraphics.length &&
            (<Selector<Graphics> type="graphics" label="Graphic" options={motherboard.relatedGraphics} onChange={selectGraphic} startingText='choose the graphic card' />)}
        {!!motherboard.relatedCases.length &&
            (<Selector<Case> type="cases" label="Case" options={motherboard.relatedCases} onChange={selectCase} startingText='choose the case' />)}
        {!!motherboard.relatedPowerSuppliers.length &&
            (<Selector<PowerSupplier> type="power-supplier" label="Power Supplier" options={motherboard.relatedPowerSuppliers} startingText='choose the power supplier' onChange={selectPowerSuppliers} />)}
        {!!motherboard.relatedStorage.length &&
            (<Selector<Storage> type="storage" label="Storage" options={motherboard.relatedStorage} startingText='choose the storage' onChange={selectStorage} />)}
    </>)
}
